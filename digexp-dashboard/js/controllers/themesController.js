...
dashboardControllers.controller('ThemeListController', ['$scope', '$route', '$location',
  function($scope, $route) {
    ...
    $scope.updateDxsyncBackgroundInterval = function(id, settings) {
      ...
      settings.password = utils.decrypt(settings.password);
    ...
    var sync = function(id) {
      ...
      $scope.themes[id].dateSynced = new Date();
      $scope.themes[id].needsToBeSynced = false;
    ...
    var updateWatchSettings = function(id) {
      ...
      if ($scope.themes[id].watchIgnore !== undefined) {
    ...
    var uploadVinyl = function(vinyl, cb, id, retries) {
      ...
      var url  = baseUrl + vinyl.relative;
      ...
      nodeFetch(url, options).then((response) => {
        if (response.status >= 400) {
          debugLogger.log("uploadVinyl: " + vinyl.relative + ": " + response.status + ", " + response.statusText);
          ...
          debugLogger.log(vinyl.relative + ": " + response.status + ", " + options.method);
          debugLogger.log("uploadVinyl " + id + ": done with " + vinyl.relative + " " + options.method + ", " + response.status);
          cb && cb(null, vinyl);
        }
    ...
    var getWebDavBaseUrl = function(id) {
      ...
      url += (config.username || config.userName) + ":" + utils.decrypt(config.password) + "@";
