...
dashboardControllers.controller('AddModuleController', ['$scope', function($scope) {
  ...
  var addFiles = function() {
    ...
    $scope.files
      ...
      .forEach(function(file) {
        mkdirp(destFolder + path.dirname(file.relative)).then(() => {
            fs.writeFileSync(destFolder + "/" + file.relative, file.contents);
        }).catch((err) => {
            console.error(err);
            throw err;
        });
  ...
  var addContributions = function() {
    ...
    async.map($scope.files, function(file, cb) {
      ...
      fs.writeFileSync($scope.configInfo.dxThemePath
  ...
  var loadExternalFiles = function() {
    ...
    $scope.newModule.urls.split(";").forEach(function(url) {
      ...
      if (url.match(/\.(js|css|html|json)$/)) {
        ...
        nodeFetch(url).then(async (response) => {
          if (response.status === 200) {
            var contents = await response.buffer();
            var urlPath = decodeURIComponent(new URL(url).pathname.substring(1));
            var v = new Vinyl({
              cwd: "./",
              path: urlPath,
              contents
            ...
            debugLogger.log("url response: " + response.status + ", " + response.statusText);
          }
  ...
  var loadExternalZip = function(url) {
    $scope.loading++;
    debugLogger.log("Loading external zip: " + url);
    nodeFetch = nodeFetch || require("node-fetch");
    nodeFetch(url).then(async (resp) => {
      debugLogger.log("done requesting " + url);
      if (resp.status !== 200) {
        debugLogger.log(url + ": " + resp.status + ", " + resp.statusText);
      } else {
        var body = await resp.buffer();
        $scope.files = $scope.files.concat(zipToVinyls(body));
        $scope.loading--;
        $scope.$apply();
      }
    }).catch(err => console.warn(err));;
