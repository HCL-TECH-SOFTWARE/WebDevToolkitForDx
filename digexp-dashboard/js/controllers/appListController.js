...
dashboardControllers.controller('AppsListController', ['$scope', '$route', '$location',
  function($scope, $route) {
          ...
          if (err) {
            ...
            try {
              ...
              var temp = {
                ...
                datePushed: utils.parseDate(spConfig.datePushed),
                dateLinted: utils.parseDate(spConfig.dateLinted),
                config: spConfig
              };
    ...
    var loadPreviewImages = function(callback) {
      ...
      var addPreviewImage = function(id, cb) {
        fs.exists(applicationsFolder + "/" + id + "/preview-image.png", function(exists) {
          if (exists) {
            $scope.apps[id] = $scope.apps[id] || {};
            $scope.apps[id].imgUrl = "file:" + applicationsFolder + "/" + id + "/preview-image.png";
          }
          cb();
        });
      };
    ...
    var loadDashConfig = function(id, cb) {
      fs.readFile(applicationsFolder + "/" + id + "/digexp-dash-config.json", function(err, data) {
        if (err) {
          ...
          $scope.apps[id].dateLinted = utils.parseDate(json.dateLinted) || $scope.apps[id].dateLinted;
          $scope.apps[id].datePushed = utils.parseDate(json.datePushed) || $scope.apps[id].datePushed;
    ...
    var saveDashConfig = function(id, app) {
      ...
      fs.writeFileSync(applicationsFolder + "/" + id + "/digexp-dash-config.json",
    ...
    $scope.lint = function(id) {
      ...
      var splintRun = function() {
        ...
        splint.run(options, function(err, out, raw) {
          ...
          $scope.apps[id].dateLinted = new Date();
    ...
    $scope.pushUpdated = function() {
      for (var key in $scope.apps) {
        utils.getModified(applicationsFolder + '/' + key, $scope.apps[key].datePushed,"/sp-build;/release;/build;sp-cmdln.log;sp-config.json",
          function(dirs){
            if(dirs.length != 0){
