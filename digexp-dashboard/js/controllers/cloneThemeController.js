...
dashboardControllers.controller('CloneThemeController', ['$scope', function($scope) {
  ...
  var uploadLocalizedMetadata = function() {
    ...
    mkdirp(themePath + "metadata")
    .catch(err => console.warn("mkdirp err: " + err.message))
    .then(() => {

      fs.writeFile(themePath + "metadata/localized_en.properties", properties, function(err) {
    ...
    .then((response) => {
      if (response) {
        if (response.status >= 400) {
          console.warn(vinyl.relative + ": " + response.status + ", " + response.statusText);
          ...
          console.log(vinyl.relative + ": " + response.status + ", " + options.method);
        }
      }
