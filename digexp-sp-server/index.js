...
var agent = url => url.protocol === 'http:' ? httpAgent : httpsAgent;
...
exports.start = function start(dir, port) {
  ...
  app.get(/^\/wps\/proxy\/http/, function(req, resp) {
    ...
    fetch(url, { headers: req.headers, compress: false, agent }).then(res => {
        resp.status(res.status);
        for (var h of res.headers.keys()) {
            resp.setHeader(h, res.headers.get(h));
        }
        res.body.pipe(resp);
    }).catch(err => {
